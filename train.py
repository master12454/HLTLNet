...
parser = argparse.ArgumentParser(description="PyTorch LTLNet train")
parser.add_argument("--model_names", default=['LTLNet'], nargs='+',
                    help="model_name: 'LTLNet'")
parser.add_argument("--dataset_names", default=['NUDT', 'NUAA','IRSTD'], nargs='+',
...
def train():
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        for idx_iter, (img, gt_mask) in enumerate(tbar):
            ...
            if img.shape[0] == 1:
                continue
            pred, L, S, D, S_history = net.forward(img)
            deimg = L + S
            reimg = deimg + D

            criterion1 = nn.L1Loss()
            criterion2 = PSNRLoss()
            loss1 = net.module.loss(pred, gt_mask)
            loss2 = net.module.loss(S, gt_mask)
            loss3 = net.module.loss(S_history, gt_mask)
            loss4 = criterion2(deimg, img)
            loss5 = criterion1(reimg, img)

            loss = 10 * loss1 + 8 * loss2 + 5 * loss3 + 0.01 * loss4 + 0.01 * loss5
